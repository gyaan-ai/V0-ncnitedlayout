-- Final migration script to ensure ALL columns exist for athlete management
-- This script is safe to run multiple times

-- First, let's see what we're working with
SELECT 'Current athletes table structure:' as status;
...
SELECT 'Final athletes table structure:' as status;
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'athletes' 
ORDER BY ordinal_position;

-- Show count of athletes
SELECT COUNT(*) as total_athletes FROM athletes;

-- Show sample data for Liam to verify
SELECT 
  first_name, 
  last_name, 
  college_commitment,
