...
export function CommitCard({
  ...
  // Replace the existing getLogoPath function with this enhanced version
  const getLogoPath = (type: "college" | "highSchool" | "club") => {
    // For Liam Hickey specifically (id === 1)
    if (id === 1) {
      switch (type) {
        case "college":
    ...
    // For other athletes, use provided logos
    switch (type) {
      case "college":
        return collegeLogo || null
      case "highSchool":
        return highSchoolLogo || null
      case "club":
        return clubLogo || null
      default:
        return null
    }
  }
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-3">
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 bg-white rounded-full flex items-center justify-center flex-shrink-0 shadow-lg border-4 border-blue-200 p-2">
                    {!logoErrors.college && getLogoPath("college") ? (
                      <img
                        src={getLogoPath("college") || "/placeholder.svg"}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col items-center">
                    <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mb-2 shadow-md border-2 border-red-200 p-1">
                      {!logoErrors.highSchool && getLogoPath("highSchool") ? (
                        <img
                          src={getLogoPath("highSchool") || "/placeholder.svg"}
                          alt={`${highSchool} logo`}
                          className="w-full h-full object-contain"
                  ...
                  {club && (
                    <div className="flex flex-col items-center">
                      <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center mb-2 shadow-md border-2 border-green-200 p-1">
                        {!logoErrors.club && getLogoPath("club") ? (
                          <img
                            src={getLogoPath("club") || "/placeholder.svg"}
                            alt={`${club} logo`}
                            className="w-full h-full object-contain"
